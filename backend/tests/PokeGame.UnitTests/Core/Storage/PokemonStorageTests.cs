...
public class PokemonStorageTests
{
  ...
  public void Given_NoOwner_When_Add_Then_ArgumentException()
  {
    var exception = Assert.Throws<ArgumentException>(() => _storage.Add(_pokemon));
    Assert.Equal("pokemon", exception.ParamName);
    Assert.StartsWith($"The Pokémon current trainer 'Id=<null>' must be 'Id={_trainer.Id}'.", exception.Message);
  }
  ...
  public void Given_AnotherOwner_When_Add_Then_ArgumentException()
  {
    ...
    var exception = Assert.Throws<ArgumentException>(() => _storage.Add(_pokemon));
    Assert.Equal("pokemon", exception.ParamName);
    Assert.StartsWith($"The Pokémon current trainer 'Id={trainer.Id}' must be 'Id={_trainer.Id}'.", exception.Message);
  }
}
