...
internal record AddInventoryItem(Guid TrainerId, Guid ItemId, AddInventoryItemPayload Payload) : ICommand<InventoryItemModel>;
...
internal class AddInventoryItemHandler : ICommandHandler<AddInventoryItem, InventoryItemModel>
{
  ...
  public async Task<InventoryItemModel> HandleAsync(AddInventoryItem command, CancellationToken cancellationToken)
  {
    ...
    AddInventoryItemPayload payload = command.Payload;
    new AddInventoryItemValidator().ValidateAndThrow(payload);
    ...
    inventory.Add(item, payload.Quantity ?? 1, actorId);
